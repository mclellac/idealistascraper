...
def euro_to_cad(euro_price: str) -> str:
    try:
        ...
        cad_price = (
            float(euro_price.replace("€", "").replace(",", "")) * conversion_rate
        )
        return f"${cad_price:,.2f}"
    except ValueError:
        raise ValueError("Invalid euro price format")
    ...
    try:
        ...
        with Progress() as progress:
            ...
            while current_url:
                ...
                next_link = soup.find(
                    "a", rel="nofollow", class_="icon-arrow-right-after", string="Next"
                )
...
def generate_html(homes: list[dict]):
    ...
    for idx, home in enumerate(homes, start=1):
        ...
        html_table += (
            f"<td><img src='{home['Photo']}' alt='{home['Title']}' width='100'></td>"
        )
    ...
    with open("html/index.html", "w") as html_file:
        ...
        html_file.write(
            "<style>table {border-collapse: collapse; width: 100%;} th, td {border: 1px solid #ddd; padding: 8px; color: #333;} th {padding-top: 12px; padding-bottom: 12px; text-align: left; background-color: #f2f2f2; color: black;}</style>\n"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--generate-html", action="store_true", help="Generate HTML file with results."
    )
    ...
    if homes:
        ...
        if args.generate_html:
            generate_html(homes)
            console.print(
                "HTML file: [magenta]html[/magenta]/[blue]index.html[/blue] generated successfully.",
                style="bold green",
            )
